        ...
        match &self.kind {
            ReplacedContentKind::Image(image) => image
                ...
                .and_then(|image| match image {
                    Image::Raster(image) => image.id,
                    Image::Vector(_, pending_image_id, _) => {
                        ...
                        let tag = self
                            .base_fragment_info
                            .tag
                            .expect("Image element must have node tag"); // TODO: Is this correct?
                        layout_context
                            .rasterize_vector_image(*pending_image_id, size, tag.node)
                            .and_then(|i| i.id)
                    },
                })
                .map(|image_key| {
