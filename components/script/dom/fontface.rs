...
use fonts::{FontTemplate, LowercaseFontFamilyName};
...
use super::bindings::error::{Error, ErrorResult};
...
pub struct FontFace {
    ...
    #[no_trace = "Does not contain managed objects"]
    #[ignore_malloc_size_of = "SourceList doesn't implement MallocSizeOf"]
...
/// string and parse it using `style` crate. Returns `None` if parsing failed.
///
...
fn parse_font_face_descriptors(
    ...
    if let Some(ref mut sources) = parsed_font_face_rule.sources {
        let supported_sources: Vec<_> = sources
            ...
            .iter()
            .rev()
            .filter(fonts::is_supported_web_font_source)
            .cloned()
            .collect();
    ...
    error_reporter
        .not_encountered_error
        .get()
        .then_some(parsed_font_face_rule)
}
...
impl FontFace {
    ...
    fn new_inherited(
        ...
        let Some(ref parsed_font_face_rule) = parse_result else {
...
macro_rules! implement_setter_for_descriptor(
    ($self:ident, $field: ident, $value: ident) => ({
        let mut new_descriptors = $self.descriptors.borrow().clone();
        new_descriptors.$field = $value;
        let global = $self.global();
        let parse_result = parse_font_face_descriptors(
                &global,
                &$self.family_name.borrow(),
                None,
                &new_descriptors
        );
        if let Some(ref parsed_font_face_rule) = parse_result {
...
impl FontFaceMethods<crate::DomTypeHolder> for FontFace {
    ...
    fn SetFamily(&self, family_name: DOMString) -> ErrorResult {
        ...
        let global = self.global();
        if parse_font_face_descriptors(&global, &family_name, None, &descriptors).is_some() {
    ...
    fn SetStyle(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, style, value)
    }
    ...
    fn SetWeight(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, weight, value)
    }
    ...
    fn SetStretch(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, stretch, value)
    }
    ...
    fn SetUnicodeRange(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, unicodeRange, value)
    }
    ...
    fn SetFeatureSettings(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, featureSettings, value)
    }
    ...
    fn SetVariationSettings(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, variationSettings, value)
    }
    ...
    fn SetDisplay(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, display, value)
    }
    ...
    fn SetAscentOverride(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, ascentOverride, value)
    }
    ...
    fn SetDescentOverride(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, descentOverride, value)
    }
    ...
    fn SetLineGapOverride(&self, value: DOMString) -> ErrorResult {
        implement_setter_for_descriptor!(self, lineGapOverride, value)
    }
    ...
    fn Load(&self) -> Rc<Promise> {
            ...
            move |family_name: LowercaseFontFamilyName, load_result: Option<_>| {
                ...
                task_source.queue(task!(resolve_font_face_load_task: move || {
                    ...
                    if let Some(font_face_set) = font_face.font_face_set.get() {
                        ...
                        font_face_set.handle_font_face_load_completed(&font_face);
                    }
                }));
            },
        );
        ...
        self.global().as_window().load_web_font_for_script(
            sources,
