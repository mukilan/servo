...
pub(crate) struct SVGSVGElement {
    ...
    // The XML source of this SVG element serialized into a base64
    // encoded `data:` url. This is cached to avoid recomputation
...
impl SVGSVGElement {
    ...
    pub(crate) fn serialize_and_cache_subtree(&self) {
        ...
        if let Ok(url) = ServoUrl::parse(&source) {
            *self.cached_serialized_data_url.borrow_mut() = Some(url);
        }
    }
}
