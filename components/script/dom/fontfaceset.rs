...
impl FontFaceSet {
    ...
    pub(super) fn handle_font_face_load_completed(&self, font_face: &FontFace) {
        if font_face.loaded() {
            if let Some(window) = DomRoot::downcast::<Window>(self.global()) {
                ...
                window.add_web_font_to_global_font_context(family_name, template);
...
impl FontFaceSetMethods<crate::DomTypeHolder> for FontFaceSet {
    ...
    fn Add(&self, font_face: &FontFace) -> DomRoot<FontFaceSet> {
        ...
        self.handle_font_face_load_completed(font_face);
