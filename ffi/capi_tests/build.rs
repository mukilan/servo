...
fn find_and_copy_dll(dll_name: &str, src_dir: &Path, dest_dir: &Path) -> bool {
    fn scan_dir(dll_name: &str, src_dir: &Path, dest_dir: &Path) -> bool {
        ...
        let paths = entries.filter_map(|entry| {
        ...
        for path in paths {
            ...
            } else if path.file_name().is_some_and(|file_name| file_name == dll_name) {
    ...
    if cfg!(target_os = "windows") {
        ...
        let copied = if search_dir.exists() {
            ["libEGL.dll", "libGLESv2.dll"]
                .iter()
                .all(|dll_name| {
                    find_and_copy_dll(dll_name, &search_dir, &test_binary_dir)
                })
        } else {
